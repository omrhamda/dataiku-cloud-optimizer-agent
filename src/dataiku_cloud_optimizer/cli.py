...
@cli.command()
@click.option("--provider", type=click.Choice(["aws", "azure", "gcp"]), required=True)
@click.option("--strategy", default="cost_optimization", help="Optimization strategy to use")
@click.option("--output", type=click.Choice(["json", "table"]), default="table")
@click.pass_context
def optimize(ctx, provider, strategy, output):
    ...
    try:
        ...
        if output == "json":
            ...
            click.echo(f"Potential Savings: ${result.savings:.2f} ({result.savings/result.current_cost*100:.1f}%)")
...
@cli.command()
@click.option("--provider", type=click.Choice(["aws", "azure", "gcp"]), help="Specific provider to get recommendations for")
@click.option("--output", type=click.Choice(["json", "table"]), default="table")
@click.pass_context
    ...
    try:
        ...
        if output == "json":
            ...
            for result in results:
                ...
                click.echo(f"  Top Recommendation: {result.recommendations[0] if result.recommendations else 'None'}")
                click.echo()
