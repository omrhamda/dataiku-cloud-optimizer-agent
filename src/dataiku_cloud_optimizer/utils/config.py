...
def get_default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "integrations": {
            "dataiku": {"url": "", "api_key": "", "project_key": ""},
            "databricks": {"workspace_url": "", "token": "", "cluster_id": ""},
        },
        "optimization": {
        ...
        if "dataiku" in integrations:
            ...
            for field in required_fields:
                if field not in dataiku_config or not dataiku_config[field]:
                    errors.append(
                        f"Dataiku integration missing required field: {field}"
                    )
        ...
        if "databricks" in integrations:
            ...
            for field in required_fields:
                if field not in databricks_config or not databricks_config[field]:
                    errors.append(
                        f"Databricks integration missing required field: {field}"
                    )
